...
class QuestionTypeController extends Controller {
  ...
  show = async (req, res) => {
    try {
      ...
      this.sendResponse(res, req.t("messages.successfully_retrived"), new QuestionType(item));
    } catch (error) {
      this.sendError(res, error.message);
    }
  };
  ...
  store = async (req, res) => {
    try {
      ...
      this.sendResponse(res, req.t("messages.successfully_created"), new QuestionType(created), true, 201);
    } catch (error) {
      this.sendError(res, error.message);
    }
  };
  ...
  update = async (req, res) => {
    try {
      ...
      this.sendResponse(res, req.t("messages.successfully_updated"), new QuestionType(updated));
    } catch (error) {
      this.sendError(res, error.message);
    }
  };
